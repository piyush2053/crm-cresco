...
export default function Dashboard() {
  ...
  return <DashboardLayout title="Dashboard"><div className="space-y-6">
    ...
    <div className="grid gap-4 xl:grid-cols-2"><TodoTable title="Prospect buyers who can order today" rows={data?.prospectsToday} navigate={navigate}/><TodoTable title="Calls / follow-ups due today" rows={data?.followUpsToday} navigate={navigate} followUp/></div>
  </div></DashboardLayout>;
}
function TodoTable({title,rows=[],navigate,followUp=false}){return <section className="overflow-hidden rounded-xl border bg-white"><div className="border-b p-4"><h3 className="font-semibold">{title}</h3></div><div className="max-h-80 overflow-auto"><table className="w-full text-sm"><thead><tr>{["Buyer","Contact",followUp?"Due date":"Monthly consumption","Owner"].map(x=><th key={x} className="border-b p-3 text-left text-xs text-muted-foreground">{x}</th>)}</tr></thead><tbody>{rows.length?rows.map(row=><tr key={row.id} onClick={()=>navigate(`/buyers?buyer=${row.id}`)} className="cursor-pointer border-b hover:bg-card"><td className="p-3 font-medium text-secondary">{row.group_name}</td><td className="p-3">{row.primary_contact_name||row.primary_contact_number||"—"}</td><td className="p-3">{followUp?row.next_call_date:(row.monthly_consumption||"—")}</td><td className="p-3">{row.lead_manager||"—"}</td></tr>):<tr><td colSpan="4" className="p-8 text-center text-muted-foreground">Nothing due today.</td></tr>}</tbody></table></div></section>}
