...
export default function BaseFreightMatrix() {
  ...
  async function load(filters = {}) {
    setLoading(true);
    try { const query = new URLSearchParams(Object.entries(filters).filter(([,value])=>String(value).trim())).toString(); const [rateRows, districtRows] = await Promise.all([api(`/logistics/base-freight${query?`?${query}`:""}`), api("/logistics/districts")]); setRows(rateRows); setDistricts(districtRows); }
  ...
  async function save(event) {
    ...
    try {
      await api("/logistics/base-freight", { method: "POST", body: JSON.stringify({ ...form, from_district_id: form.from, to_district_id: form.to }) });
      setForm(emptyForm);
      setOpen(false);
      setEditing(false);
  ...
  function edit(row) {
    setForm({ from: row.from_district_id, to: row.to_district_id, load: row.quantity_kg, freight: row.base_freight_per_kg, remarks: "Rate edited from Freight Rates" });
    setEditing(true);
    setOpen(true);
  }
  ...
  return <section className="overflow-visible rounded-xl border">
    ...
    {appliedSearch&&!searchDirty&&<div className="flex items-center gap-2 border-b bg-card/30 px-4 py-2 text-xs"><span className="text-muted-foreground">Searched results:</span><span className="inline-flex items-center gap-1 rounded-full border border-accent/30 bg-accent/10 px-2.5 py-1 font-medium">{appliedSearch.from?districtLabel(appliedSearch.from):"Any From"} → {appliedSearch.to?districtLabel(appliedSearch.to):"Any To"}<button type="button" onClick={clearSearch} className="ml-1 rounded-full hover:bg-white" aria-label="Clear searched results"><X className="h-3.5 w-3.5"/></button></span></div>}
